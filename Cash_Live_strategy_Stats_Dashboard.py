...
# Option 1: Sum StockInvestment if available and non-empty
    ...
    async def run_dashboard_async(self):
        ...
        .kpi-value.investment, .kpi-value.open, .kpi-value.closed {
            color: #000000; /* Black color for Investment Amount, Open Positions, Closed Positions */
        }
        .kpi-value.profit {
...
""", unsafe_allow_html=True)
        ...
        if st.session_state.data_fetched:
            ...
            investment = get_kpi_value(stats_df, "Investment Amount", as_int=True)
            ...
            st.markdown(
                ...
                self.render_kpi_card("Investment", investment, "investment"),
            ...
            if stats_df.empty:
                ...
                for stat in stats_list:
                    label = stat["Metric"]
                    value = stat["Value"]
                    # Exclude Investment Amount, Number of Open Positions, Number of Closed Positions
                    if label in ["Investment Amount", "Number of Open Positions", "Number of Closed Positions"]:
                        continue
